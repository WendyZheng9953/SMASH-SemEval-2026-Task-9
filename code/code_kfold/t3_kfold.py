...
def load_fold_train(base_dir: Path, language: str, fold: int) -> pd.DataFrame:
    p = base_dir / "subtask3_kfold" / language / f"fold_{fold}" / "train.csv"
    if not p.exists():
...
def load_fold_val(base_dir: Path, language: str, fold: int) -> pd.DataFrame:
    p = base_dir / "subtask3_kfold" / language / f"fold_{fold}" / "val.csv"
    if not p.exists():
...
def run_one_fold(args, fold: int):
    ...
    print(f"\n=== Task3 | Language={args.language} | Fold={fold} | include_other_languages={args.include_other_languages} ===")
...
def main():
    ...
    parser.add_argument("--include_other_languages", action="store_true",
                        help="If set, merge ../dev_phase/subtask3/train/*.csv excluding <language>.csv into training data")
    parser.add_argument("--fold", type=int, default=-1,
                        help="Fold number 0-4. Use -1 to run all 5 folds.")

    parser.add_argument("--base_dir", type=str, default="../dev_phase",
                        help="Base folder that contains subtask3/ and subtask3_kfold/")
    parser.add_argument("--output_dir", type=str, default="fine-tuned_BERT/task3_kfold/",
