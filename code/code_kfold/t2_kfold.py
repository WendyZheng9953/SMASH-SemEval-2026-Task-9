...
def load_fold_train(base_dir: Path, language: str, fold: int) -> pd.DataFrame:
    p = base_dir / "subtask2_kfold" / language / f"fold_{fold}" / "train.csv"
    if not p.exists():
...
def load_fold_val(base_dir: Path, language: str, fold: int) -> pd.DataFrame:
    p = base_dir / "subtask2_kfold" / language / f"fold_{fold}" / "val.csv"
    if not p.exists():
        raise FileNotFoundError(f"Missing val file: {p}")
    df = pd.read_csv(p)

    if "text" not in df.columns:
        raise ValueError(f"{p} missing column: text")

    return add_language_prefix(df, language)

# Discarded
def load_other_languages_train(base_dir: Path, exclude_language: str, fold: int) -> pd.DataFrame:
    kfold_root = base_dir / "subtask2_kfold"
    if not kfold_root.exists():
        raise FileNotFoundError(f"Missing kfold folder: {kfold_root}")

    dfs = []
    for lang_dir in sorted([p for p in kfold_root.iterdir() if p.is_dir()]):
        lang = lang_dir.name
        train_path = lang_dir / f"fold_{fold}" / "train.csv"
...
def run_one_fold(args, fold: int):
    ...
    print(f"\n=== Task2 | Language={args.language} | Fold={fold} | include_other_languages={args.include_other_languages} ===")
    print(f"Loading model/tokenizer from: {load_source}")
    tokenizer = AutoTokenizer.from_pretrained(load_source)

    train_dataset = PolarizationTypeDataset(train_texts, train_labels.tolist(), tokenizer, max_length=args.max_length)
    val_dataset = PolarizationTypeDataset(val_texts, dummy_labels.tolist(), tokenizer, max_length=args.max_length)
...
def main():
    ...
    parser.add_argument("--include_other_languages", action="store_true",
                        help="If set, merge ../dev_phase/subtask2/train/*.csv excluding <language>.csv into training data")
    parser.add_argument("--fold", type=int, default=-1,
                        help="Fold number 0-4. Use -1 to run all 5 folds.")

    parser.add_argument("--base_dir", type=str, default="../dev_phase",
                        help="Base folder that contains subtask2/ and subtask2_kfold/")
    parser.add_argument("--output_dir", type=str, default="fine-tuned_BERT/task2_kfold/",
