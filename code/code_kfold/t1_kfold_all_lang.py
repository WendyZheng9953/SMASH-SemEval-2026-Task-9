...
def load_all_languages_fold_train(base_dir: Path, fold: int) -> pd.DataFrame:
    ...
    kfold_root = base_dir / "subtask1_kfold"
    ...
    for lang in list_kfold_languages(kfold_root):
        ...
        if "text" not in df.columns or "polarization" not in df.columns:
            raise ValueError(f"{train_path} must contain columns: text, polarization")
...
def load_all_languages_fold_val(base_dir: Path, fold: int) -> dict[str, pd.DataFrame]:
    ...
    for lang in list_kfold_languages(kfold_root):
        ...
        if "text" not in df.columns:
            raise ValueError(f"{val_path} must contain column: text")
...
def save_val_with_logits(base_dir: Path, language: str, fold: int, val_df: pd.DataFrame, logits: np.ndarray):
    ...
    out_path = base_dir / "subtask1_kfold" / language / f"fold_{fold}_val.csv"
...
def run_one_fold(args, fold: int):
    ...
    print(f"\n=== Fold={fold} | Train on ALL languages ===")
    ...
    for lang, val_df in val_by_lang.items():
        ...
        save_val_with_logits(Path(fold_out_dir), lang, fold, val_df, logits)
...
def main():
    ...
    parser.add_argument("--base_dir", type=str, default="../dev_phase",
                        help="Base folder that contains subtask1_kfold/")
    parser.add_argument("--output_dir", type=str, default="fine-tuned_BERT/task1_kfold_all_lang/",
